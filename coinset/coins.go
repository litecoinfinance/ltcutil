...
func (cs *CoinSet) TotalValue() (value ltfnutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue ltfnutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue ltfnutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount ltfnutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue ltfnutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue ltfnutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue ltfnutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue ltfnutil.Amount, coins []Coin) (Coins, error) {
...
// ltfnutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() ltfnutil.Amount {
	return ltfnutil.Amount(c.txOut().Value)
}
